...
function C_PieceD(piece){
  ...
  if (piece === 10) return 16;
  if (piece === 13) return 27;
  ...
  for (const tryOrder of ordersToTry){
    for (let sliceNo=1; sliceNo<=SLICE_TRIES; sliceNo++){
      ...
      for (const typeName of tryOrder){
        ...
        let limit = Math.min(MAX_CANDS_PER_TYPE, localAll.length);
        if (limit < 1) limit = 1;
        ...
        if ((!res.list || res.list.length===0) && ALLOW_SOFT){
          ...
          let limit2 = Math.min(MAX_CANDS_PER_TYPE, localAll2.length);
          if (limit2 < 1) limit2 = 1;

          const startAt2 = 1 + (sliceNo - 1) * SLICE_SHIFT_STEP;
          const localCands2 = sliceCandidates(localAll2, startAt2, limit2);
        ...
        remX -= sumX;
        remY -= sumY;
        usedType[typeName].c += sumC2;
        usedType[typeName].d += sumD2;
      }
...
btn.addEventListener("click", (e) => {
  ...
  setTimeout(() => {
    try{
      ...
      const candidates = genCandidatesByD(targetD);
